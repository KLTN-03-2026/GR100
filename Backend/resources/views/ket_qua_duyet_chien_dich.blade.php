...
<head>
    ...
    <title>Thông báo chiến dịch</title>
...
@php
    $status = $data['loai'] ?? 'duyet';
    $themes = [
        'duyet' => ['bg' => '#ECFDF5', 'accent' => '#059669', 'title' => 'Chiến dịch đã được duyệt'],
        'tu_choi' => ['bg' => '#FEF2F2', 'accent' => '#DC2626', 'title' => 'Chiến dịch bị từ chối'],
        'duyet_huy' => ['bg' => '#FEF2F2', 'accent' => '#DC2626', 'title' => 'Yêu cầu hủy đã được duyệt'],
        'tu_choi_huy' => ['bg' => '#EFF6FF', 'accent' => '#2563EB', 'title' => 'Yêu cầu hủy bị từ chối'],
        'cap_nhat_trang_thai' => ['bg' => '#F5F3FF', 'accent' => '#7C3AED', 'title' => 'Trạng thái chiến dịch đã được cập nhật'],
    ];
    $theme = $themes[$status] ?? $themes['duyet'];
@endphp
            ...
            <table width="600" cellpadding="0" cellspacing="0" role="presentation" style="background:#ffffff;border-radius:16px;overflow:hidden;max-width:600px;width:100%;box-shadow:0 12px 32px rgba(15,23,42,0.08);">
                <tr>
                    <td style="padding:32px;background:{{ $theme['bg'] }};border-bottom:1px solid #E5E7EB;">
                        ...
                        <p style="margin:8px 0 0;font-size:14px;line-height:22px;color:#4B5563;">Thông báo từ hệ thống kiểm duyệt chiến dịch</p>
                    </td>
                </tr>
                <tr>
                    <td style="padding:28px 32px;">
                        <p style="margin:0 0 16px;font-size:15px;line-height:24px;color:#374151;">Xin chào <strong>{{ $data['ho_ten'] ?? 'bạn' }}</strong>,</p>
                        <p style="margin:0 0 20px;font-size:15px;line-height:24px;color:#4B5563;">{{ $data['thong_diep'] ?? 'Chiến dịch của bạn vừa có cập nhật mới từ kiểm duyệt viên.' }}</p>
                        ...
                        <div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:12px;padding:18px 20px;margin-bottom:20px;">
                            ...
                            <p style="margin:0;font-size:14px;color:#374151;">Trạng thái hiện tại: <strong>{{ $data['trang_thai_hien_tai'] ?? '—' }}</strong></p>
                        </div>

                        @if(!empty($data['ly_do']))
                        <div style="background:#FFF7ED;border-left:4px solid #F97316;border-radius:8px;padding:14px 16px;margin-bottom:20px;">
                            <p style="margin:0 0 6px;font-size:13px;font-weight:600;color:#9A3412;">Lý do / phản hồi từ kiểm duyệt viên</p>
                            <p style="margin:0;font-size:14px;line-height:22px;color:#7C2D12;">{{ $data['ly_do'] }}</p>
                        </div>
                        @endif
